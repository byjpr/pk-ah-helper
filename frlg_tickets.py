#!/usr/bin/env python3
"""
frlg_tickets.py — enable the Aurora Ticket and Mystic Ticket events on a
Pokemon FireRed / LeafGreen save file (.sav).
...
All offsets/constants are taken from the pret/pokefirered decompilation:
  FLAG_RECEIVED_AURORA_TICKET      = 0x2A7
  FLAG_RECEIVED_MYSTIC_TICKET      = 0x2A8
  FLAG_ENABLE_SHIP_NAVEL_ROCK      = 0x84A   (SYS_FLAGS 0x800 + 0x4A)
  FLAG_ENABLE_SHIP_BIRTH_ISLAND    = 0x84B
  FLAG_WORLD_MAP_NAVEL_ROCK        = 0x8B5
  FLAG_WORLD_MAP_BIRTH_ISLAND      = 0x8C2
  ITEM_MYSTIC_TICKET               = 370
  ITEM_AURORA_TICKET               = 371
  SaveBlock1.bagPocket_KeyItems    @ 0x03B8 (30 slots of {u16 id, u16 qty})
  SaveBlock1.flags                 @ 0x0EE0
  SaveBlock2.encryptionKey         @ 0x0F20
...
SIZEOF_SAVEBLOCK2 = 0x0F24  # 3876
SIZEOF_SAVEBLOCK1 = 0x3D68  # 15720
SIZEOF_PKMN_STORAGE = 33744

# bytes of each section actually covered by its checksum
...
assert CHECKSUM_SIZES[4] == 3816 and CHECKSUM_SIZES[13] == 2000

SB1_FLAGS_OFFSET = 0x0EE0
SB1_KEYITEMS_OFFSET = 0x03B8
KEYITEMS_SLOTS = 30
SB2_ENCRYPTION_KEY_OFFSET = 0x0F20
...
class Slot:
    ...
    def sb1_locate(self, sb1_off: int):
        """Map a SaveBlock1 offset to (sector, offset-within-sector-data)."""
    ...
    def game_code(self) -> int:
        # SaveBlock2 +0xAC: 0 in Ruby/Sapphire, 1 in FRLG, random key in Emerald
...
def load_slots(buf: bytearray):
    ...
    if not slots:
        sys.exit("error: no valid save slot found — is this a raw 128 KiB "
                 "FireRed/LeafGreen .sav (not a savestate)?")
    return max(slots, key=lambda s: (s.counter + 1) & 0xFFFFFFFF)


def main():
    ap = argparse.ArgumentParser(description="Enable Aurora/Mystic Ticket events on a FRLG save.")
    ap.add_argument("save", help="path to the .sav file")
    ap.add_argument("--aurora", action="store_true", help="Aurora Ticket only")
    ap.add_argument("--mystic", action="store_true", help="Mystic Ticket only")
    ...
    ap.add_argument("--force", action="store_true", help="skip the FRLG game check")
    args = ap.parse_args()
    ...
    code = slot.game_code()
    if code != 1 and not args.force:
        guess = {0: "Ruby/Sapphire"}.get(code, "Emerald (or unknown)")
        sys.exit(f"error: this save doesn't look like FireRed/LeafGreen "
                 f"(game code field = 0x{code:08X}, likely {guess}). "
                 "Use --force to override.")

    wanted = [k for k in ("aurora", "mystic") if getattr(args, k)] or ["aurora", "mystic"]

    if args.info:
        print(f"active slot counter: {slot.counter}")
        for name in ("aurora", "mystic"):
            t = TICKETS[name]
    ...
    with open(out, "wb") as f:
        f.write(buf)
    print(f"written: {out}")
    print("\nIn game: talk to the sailor at the Seagallop pier in Vermilion City "
          "(or any Seagallop pier) and pick Birth Island / Navel Rock.")
